...
class LinearClassifier(Classifier):
  ...
  def backward(self, data, count):
    ...
    """
    L = softmax.map(lambda (x, (L, df)): L).reduce(lambda arg1, arg2: arg1 + arg2)
    ...
    """
    backpropagation = softmax.map(lambda (x, (L, df)): linear_backward(df, x, self.A))
    ...
    """
    dLdA = backpropagation.map(lambda (DX, DA, db): DA).reduce(lambda arg1, arg2: arg1 + arg2)
    dLdb = backpropagation.map(lambda (DX, DA, db): db).reduce(lambda arg1, arg2: arg1 + arg2)
