...
class NNClassifier(Classifier):
  ...
  def forward(self, data):
    ...
    return data.map(lambda (k, (x, y)): (k, (x, [linear_forward(x, A1, b1), \
            ReLU_forward(linear_forward(x, A1, b1)), linear_forward(ReLU_forward( \
            linear_forward(x, A1, b1)), A3, b3)], y)))
  ...
  def backward(self, data, count):
    ...
    """
    L = softmax.map(lambda (x, layers, (L, df)): L).reduce(lambda arg1, arg2: arg1 + arg2) # replace it with your code1

    """ regularization """
    L += 0.5 * self.lam * (np.sum(self.A1*self.A1) + np.sum(self.A3*self.A3))

    """ Todo: Implement backpropagation for Layer 3 """
    backpropagation3 = softmax.map(lambda (x, layers, (L, df)): (x, layers, df, linear_backward(df, layers[1], self.A3)))

    """ Todo: Compute the gradient on A3 and b3 """
    dLdA3 = backpropagation3.map(lambda (x, layers, df, (DX, DA, db)): DA).reduce(lambda arg1, arg2: arg1 + arg2) # replace it with your code
    dLdb3 = backpropagation3.map(lambda (x, layers, df, (DX, DA, db)): db).reduce(lambda arg1, arg2: arg1 + arg2) # replace it with your code

    """ Todo: Implement backpropagation for Layer 2 """
    backpropagation2 = backpropagation3.map(lambda (x, layers, df, (DX, DA, db)): (x, layers, ReLU_backward(DX, layers[0])))

    """ Todo: Implmenet backpropagation for Layer 1 """
    backpropagation1 = backpropagation2.map(lambda (x, layers, df): linear_backward(df, x, self.A1))

    """ Todo: Compute the gradient on A1 and b1 """
    dLdA1 = backpropagation1.map(lambda (DX, DA, db): DA).reduce(lambda arg1, arg2: arg1 + arg2) # replace it with your code
    dLdb1 = backpropagation1.map(lambda (DX, DA, db): db).reduce(lambda arg1, arg2: arg1 + arg2) # replace it with your code
